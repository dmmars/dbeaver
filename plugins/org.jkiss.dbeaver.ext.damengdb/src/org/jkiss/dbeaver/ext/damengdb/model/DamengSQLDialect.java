...
import org.jkiss.dbeaver.ext.damengdb.data.DamengBinaryFormatter;
 ...
 * Dameng SQL dialect
 */
public class DamengSQLDialect extends JDBCSQLDialect
    implements SQLDataTypeConverter, SQLDialectDDLExtension, SQLDialectSchemaController {

    private static final Log log = Log.getLog(DamengSQLDialect.class);

    private static final String[] EXEC_KEYWORDS = new String[] {"call"};

    private static final String[] DM_NON_TRANSACTIONAL_KEYWORDS = ArrayUtils.concatArrays(
        BasicSQLDialect.NON_TRANSACTIONAL_KEYWORDS,
        new String[] {"CREATE", "ALTER", "DROP", "ANALYZE", "VALIDATE",});

    private static final String[][] DM_BEGIN_END_BLOCK = new String[][] {
        {SQLConstants.BLOCK_BEGIN, SQLConstants.BLOCK_END}, {"LOOP", SQLConstants.BLOCK_END + " LOOP"},
        {SQLConstants.KEYWORD_CASE, SQLConstants.BLOCK_END + " " + SQLConstants.KEYWORD_CASE},};

    private static final String[] DM_BLOCK_HEADERS = new String[] {"DECLARE", "PACKAGE"};

    private static final String[] DM_INNER_BLOCK_PREFIXES = new String[] {"AS", "IS",};

    private static final String[] OTHER_TYPES_FUNCTIONS = {
        // functions without parentheses #8710
        "CURRENT_DATE", "CURRENT_TIMESTAMP", "DBTIMEZONE", "SESSIONTIMEZONE", "SYSDATE", "SYSTIMESTAMP"};

    private static final String[] ADVANCED_KEYWORDS = {"REPLACE", "PACKAGE", "FUNCTION", "TYPE", "BODY", "RECORD",
        "TRIGGER", "MATERIALIZED", "IF", "EACH", "RETURN", "WRAPPED", "AFTER", "BEFORE", "DATABASE", "ANALYZE",
        "VALIDATE", "STRUCTURE", "COMPUTE", "STATISTICS", "LOOP", "WHILE", "BULK", "ELSIF", "EXIT", "SUBPARTITION",
        "TEMPFILE", "DATAFILE", "TABLESPACE", "EXPLAIN", "LINK", "PUBLIC", "OPERATOR", "SPATIAL", "BITMAP"};
    ...
    public void initDriverSettings(JDBCSession session, JDBCDataSource dataSource, JDBCDatabaseMetaData metaData) {
        ...
        addFunctions(Arrays.asList("SUBSTR", "APPROX_COUNT_DISTINCT", "REGEXP_SUBSTR", "REGEXP_INSTR", "REGEXP_REPLACE",
            ...
            // Number Functions:
            "BITAND", "COSH", "NANVL", "REMAINDER", "SINH", "TANH", "TRUNC",

            // Character Functions Returning Character Values:
            "CHR", "INITCAP", "LPAD", "NLS_INITCAP", "NLS_LOWER", "NLSSORT", "NLS_UPPER", "RPAD", "REVERSE",
            ...
            "INSTR", "INSTRB", "INSTRC", "INSTR2", "INSTR4", "LENGTHB", "LENGTH",

            // Datetime Functions:
            "ADD_MONTHS", "FROM_TZ", "LAST_DAY", "MONTHS_BETWEEN", "NEW_TIME", "NEXT_DAY", "NUMTODSINTERVAL",
            "NUMTOYMINTERVAL", "SYS_EXTRACT_UTC", "TO_CHAR", "TO_TIMESTAMP", "TO_TIMESTAMP_TZ", "TO_DSINTERVAL",
            ...
            "ASCIISTR", "BIN_TO_NUM", "CHARTOROWID", "COMPOSE", "DECOMPOSE", "HEXTORAW", "NUMTODSINTERVAL",
            "NUMTOYMINTERVAL", "RAWTOHEX", "RAWTONHEX", "ROWIDTOCHAR", "ROWIDTONCHAR", "SCN_TO_TIMESTAMP",
            "TIMESTAMP_TO_SCN", "TO_BINARY_DOUBLE", "TO_BINARY_FLOAT", "TO_CHAR", "TO_CLOB", "TO_DATE",
            "TO_DSINTERVAL", "TO_LOB", "TO_MULTI_BYTE", "TO_NCHAR", "TO_NCLOB", "TO_NUMBER", "TO_DSINTERVAL",
            "TO_SINGLE_BYTE", "TO_TIMESTAMP", "TO_TIMESTAMP_TZ", "TO_YMINTERVAL", "TO_YMINTERVAL", "UNISTR",
            ...
            "CLUSTER_ID", "CLUSTER_PROBABILITY", "CLUSTER_SET", "FEATURE_ID", "FEATURE_SET", "FEATURE_VALUE",
            ...
            "INSERTXMLBEFORE", "PATH", "SYS_DBURIGEN", "SYS_XMLAGG", "SYS_XMLGEN", "UPDATEXML", "XMLAGG",
            "XMLCDATA", "XMLCOLATTVAL", "XMLCOMMENT", "XMLCONCAT", "XMLFOREST", "XMLPARSE", "XMLPI", "XMLQUERY",
            "XMLROOT", "XMLSEQUENCE", "XMLSERIALIZE", "XMLTABLE", "XMLTRANSFORM",
            ...
            "SYS_CONTEXT", "SYS_GUID", "SYS_TYPEID", "UID", "USERENV",

            // Aggregate Functions:
            "CORR_S", "CORR_K", "FIRST", "GROUP_ID", "GROUPING_ID", "LAST", "MEDIAN", "STATS_BINOMIAL_TEST",
            "STATS_CROSSTAB", "STATS_F_TEST", "STATS_KS_TEST", "STATS_MODE", "STATS_MW_TEST", "STATS_ONE_WAY_ANOVA",
            "STATS_T_TEST_ONE", "STATS_T_TEST_PAIRED", "STATS_T_TEST_INDEP", "STATS_T_TEST_INDEPU",
            "STATS_WSR_TEST", "STDDEV", "VARIANCE",

            // Analytic Functions:
            "FIRST", "FIRST_VALUE", "LAG", "LAST", "LAST_VALUE", "LEAD", "NTILE", "RATIO_TO_REPORT", "STDDEV",
            "VARIANCE", "COALESCE",
    ...
    protected void loadDataTypesFromDatabase(JDBCDataSource dataSource) {
        ...
        addDataTypes(DamengDataType.PREDEFINED_TYPES.keySet());
    }
    ...
    public String[] getInnerBlockPrefixes() {
        return DM_INNER_BLOCK_PREFIXES;
    }
    ...
    public String escapeScriptValue(DBSTypedObject attribute, @NotNull Object value, @NotNull String strValue) {
        if (CommonUtils.isNaN(value) || CommonUtils.isInfinite(value)) {
            // These special values should be quoted, as shown in the example
            // below
            // https://docs.dm.com/cd/B19306_01/server.102/b14200/functions090.htm
    ...
    public DBDBinaryFormatter getNativeBinaryFormatter() {
        return DamengBinaryFormatter.INSTANCE;
    }
    ...
    public boolean isDisableScriptEscapeProcessing() {
        return preferenceStore == null
            || preferenceStore.getBoolean(DamengConstants.PREF_DISABLE_SCRIPT_ESCAPE_PROCESSING);
    }
    ...
    public String getColumnTypeModifiers(@NotNull DBPDataSource dataSource, @NotNull DBSTypedObject column,
        ...
        Integer scale;
        switch (typeName) {
            case DamengConstants.TYPE_NUMBER:
            case DamengConstants.TYPE_DECIMAL:
                ...
                if (precision == 0 || precision > DamengConstants.NUMERIC_MAX_PRECISION) {
                    precision = DamengConstants.NUMERIC_MAX_PRECISION;
                }
                if (scale != null || precision > 0) {
                    // 38 - is default precision value. And we can not add scale
                    // here.
                    // It will be changed to 0 automatically after table
                    // creation from the Dm side.
                    return "(" + (precision > 0 ? precision : "38") + (scale != null ? "," + scale : "") + ")";
                }
                break;
            case DamengConstants.TYPE_INTERVAL_DAY_SECOND:
                // This interval type has fractional seconds precision. In
                // bounds from 0 to 9. We can show this parameter.
                // FIXME: This type has day precision inside type name. Like
                // INTERVAL DAY(2) TO SECOND(6). So far we can't show it (But we
                // do it in Column Manager)
                scale = column.getScale();
                if (scale == null) {
                    return "";
                }
                if (scale < 0 || scale > 9) {
                    scale = DamengConstants.INTERVAL_DEFAULT_SECONDS_PRECISION;
                }
                return "(" + scale + ")";
            case DamengConstants.TYPE_NAME_BFILE:
            case DamengConstants.TYPE_NAME_CFILE:
            case DamengConstants.TYPE_CONTENT_POINTER:
            case DamengConstants.TYPE_LONG:
            case DamengConstants.TYPE_LONG_RAW:
            case DamengConstants.TYPE_OCTET:
            case DamengConstants.TYPE_INTERVAL_YEAR_MONTH:
    ...
    public String convertExternalDataType(@NotNull SQLDialect sourceDialect, @NotNull DBSTypedObject sourceTypedObject,
        ...
        switch (externalTypeName) {
            case "VARCHAR":
                ...
                // See
                // https://docs.dm.com/en/database/dm/dm-database/19/sqlrf/Data-Types.html#GUID-DF7E10FC-A461-4325-A295-3FD4D150809E
                localDataType = DamengConstants.TYPE_NAME_VARCHAR2;
                if (sourceTypedObject.getMaxLength() > 0 && sourceTypedObject.getMaxLength() != Integer.MAX_VALUE
            ...
            case "XML":
            case "XMLTYPE":
                localDataType = DamengConstants.TYPE_FQ_XML;
                break;
            case "JSON":
            case "JSONB":
                localDataType = "JSON";
            ...
            case "GEOGRAPHY":
            case "ST_GEOMETRY":
                localDataType = DamengConstants.TYPE_DMGEO2;
                break;
            case "NUMERIC":
                localDataType = DamengConstants.TYPE_NUMBER;
    ...
    private SQLTokenPredicateSet makeDialectSkipTokenPredicates(JDBCDataSource dataSource) {
        ...
        TokenPredicateSet conditions = TokenPredicateSet.of(
            ...
            new TokenPredicatesCondition(SQLParserActionKind.BEGIN_BLOCK, tt.sequence(),
    ...
    public String getBigIntegerType() {
        return DamengConstants.TYPE_NUMBER;
    }

    @NotNull
    @Override
    public String getClobDataType() {
        return DamengConstants.TYPE_CLOB;
    }
    ...
    public String getUuidDataType() {
        return DamengConstants.TYPE_UUID;
    }

    @NotNull
    @Override
    public String getBooleanDataType() {
        return DamengConstants.TYPE_BOOLEAN;
    }

    @NotNull
    @Override
    public String getAlterColumnOperation() {
        return DamengConstants.OPERATION_MODIFY;
    }
