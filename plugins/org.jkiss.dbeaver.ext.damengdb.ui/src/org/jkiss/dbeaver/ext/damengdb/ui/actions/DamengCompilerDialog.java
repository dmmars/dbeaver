...
/**
 * DamengCompilerDialog
 */
public class DamengCompilerDialog extends BaseDialog {
	
    private static final Log log = Log.getLog(DamengCompilerDialog.class);

    private static final int COMPILE_ID = 1000;

    private static final int COMPILE_ALL_ID = 1001;

    private java.util.List<DamengSourceObject> compileUnits;

    private TableViewer unitTable;

    private ObjectCompilerLogViewer compileLog;

    public DamengCompilerDialog(Shell shell, java.util.List<DamengSourceObject> compileUnits) {
        super(shell, DamengUIMessages.views_dameng_compiler_dialog_title, null);
    ...
    protected Composite createDialogArea(Composite parent) {
...

        {
            ...
            ViewerColumnController columnController = new ViewerColumnController("DamengCompilerDialog", unitTable);
            columnController.addColumn(DamengUIMessages.views_dameng_compiler_dialog_column_name, null, SWT.NONE, true,
                true, new CellLabelProvider() {
                    @Override
            ...
            columnController.addColumn(DamengUIMessages.views_dameng_compiler_dialog_column_type, null, SWT.NONE, true,
                true, new CellLabelProvider() {
                    @Override
                    public void update(ViewerCell cell) {
                        DBSObject unit = (DBSObject) cell.getElement();
            ...
            unitTable.addDoubleClickListener(event -> {
                ...
                if (!selection.isEmpty()) {
                    DamengSourceObject unit = (DamengSourceObject) selection.getFirstElement();
    ...
    protected void createButtonsForButtonBar(Composite parent) {
        ...
        createButton(parent, COMPILE_ID, DamengUIMessages.views_dameng_compiler_dialog_button_compile, false)
            .setEnabled(false);
        createButton(parent, COMPILE_ALL_ID, DamengUIMessages.views_dameng_compiler_dialog_button_compile_all, true);
    ...
    protected void buttonPressed(int buttonId) {
        final List<DamengSourceObject> toCompile;
    ...
    private void performCompilation(DBRProgressMonitor monitor, List<DamengSourceObject> units) {
        compileLog.layoutLog();
        for (DamengSourceObject unit : units) {
            if (monitor.isCanceled()) {
                break;
            }
            final String message = NLS.bind(DamengUIMessages.views_dameng_compiler_dialog_message_compile_unit,
            ...
            compileLog.info(!success ? DamengUIMessages.views_dameng_compiler_dialog_message_compilation_error
                : DamengUIMessages.views_dameng_compiler_dialog_message_compilation_success);
